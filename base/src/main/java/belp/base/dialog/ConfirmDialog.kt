...
import belp.base.R
import belp.base.databinding.DialogConfirmBinding
...
class ConfirmDialog(context: Context) : BaseDialog<DialogConfirmBinding>(context) {
    ...
    private fun setupTexts() {
        ...
        mViewDataBinding.btnYes.text =
                if (!TextUtils.isEmpty(mYesButtonText)) mYesButtonText else context.getString(R.string.yes)
        mViewDataBinding.btnNo.text =
                if (!TextUtils.isEmpty(mNoButtonText)) mNoButtonText else context.getString(R.string.cancel)
        if (!TextUtils.isEmpty(mTitle)) {
            mViewDataBinding.tvTitle.text = mTitle
            return
        }
        val title = when (mConfirmType) {
            ConfirmType.ERROR -> context.getString(R.string.error)
            ConfirmType.SUCCESS -> context.getString(R.string.success)
    ...
    fun show(
        onYesListener: OnYesListener? = null, onNoListener: OnNoListener? = null,
        ...
        noText: String = context.getString(R.string.no), tag: String? = null, cancelable: Boolean = true
    ) {
